...
class GiveawayLoginController extends Controller
{
    ...
    public function store(Request $request)
    {
       ...
       if(!$user) {
              return redirect( $giveaway->link)
                ->with('toast_error', 'User does not exist, register to participate in the giveaway')
                ->withInput();
        ...
        $participate = GiveAwayContacts::where('giveaway_id',$request->giveaway_id)->where('user_id',$user->userId)->first();
        if($participate) {
             return back()
                ->with('toast_error', 'User already participate in the giveaway')
                ->withInput();
        }
        // dd($participate);
        
       
        //check if user has same session, so he cannot participate twice.
        ...
        if(count($giveaway->all_numbers ?? []) / $giveaway->part_no == 1) {
             return back()
                ->with('toast_error', 'Giveaway Ended Already!')
                ->withInput();
        }
        if (count($existingNumbers) >= $giveaway->part_no) {
            ...
            if (in_array($randomNumber, $giveaway->lucky_numbers)) {
                ...
                //
                   ...
                   DB::table('funds')->insert([
                            ...
                            'email' => $user->email,
                            'amount' => $amount,
                            'paymentType' => 'Giveaway Winner',
                            'accountName' => 'Giveaway',
                            'accountNumber' => 'Giveaway',
                            'bankName' => 'Giveaway',
                            'status' => 'success',
                            'Admin' => 'None',
                            "created_at" => date('Y-m-d H:i:s'),
                        ...
                        DB::table('transactions')->insert([
                            ...
                            'transactionService' => 'Giveaway Winner',
                            'status' => 'CONFIRM',
                            'paymentMethod' => 'wallet',
                            'Admin' => 'None',
                            "created_at" => date('Y-m-d H:i:s'),
         ...
         //save giveaway into session so users with the same phone cannot participate twice.
        ...
        //      return redirect()->route('my_giveaway');
