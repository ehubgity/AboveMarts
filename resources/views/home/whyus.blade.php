 <!-- WHY CHOOSE US
        ================================================== -->
        <section>
            <div class="container z-index-2 position-relative">
              <div class="section-heading">
                <span class="subtitle">Why Choose Us</span>
                <h2>
                  Best Digital Solution &
                  <span class="font-weight-400">Wealth Building Platform </span>
                </h2>
              </div>
            </div>
            <div class="container-fluid">
              <div class="row justify-content-center z-index-2 position-relative">
                <div class="col-xxl-9">
                  <div class="row align-items-md-center">
                    <div
                      class="col-sm-6 col-lg-4 wow fadeInUp"
                      data-wow-delay="200ms"
                    >
                      <div class="d-flex ms-lg-6 mb-1-9 mb-md-6">
                        <div
                          class="flex-grow-1 text-lg-end ms-4 ms-lg-0 me-lg-1-9 order-2 order-lg-1"
                        >
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Quality Services</a>
                          </h4>
                          <p class="mb-0">
                            AboveMarts provides you with to a high quality service to help improve your productivity.
                          </p>
                        </div>
                        <div
                          class="flex-shrink-0 position-relative order-1 order-lg-2"
                        >
                          <div class="icon-box">
                            <img src="img/icons/icon-07.png" alt="..." />
                          </div>
                        </div>
                      </div>
                      <div class="d-flex mb-1-9 mb-md-6 me-lg-6">
                        <div
                          class="flex-grow-1 text-lg-end ms-4 ms-lg-0 me-lg-1-9 order-2 order-lg-1"
                        >
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Global Delivery</a>
                          </h4>
                          <p class="mb-0">
                            Our distribution network is both national and global - the best part is that, it is very affordable
                          </p>
                        </div>
                        <div
                          class="flex-shrink-0 position-relative order-1 order-lg-2"
                        >
                          <div class="icon-box">
                            <img src="img/icons/icon-08.png" alt="..." />
                          </div>
                        </div>
                      </div>
                      <div class="d-flex mb-1-9 mb-md-0 ms-lg-6">
                        <div
                          class="flex-grow-1 text-lg-end ms-4 ms-lg-0 me-lg-1-9 order-2 order-lg-1"
                        >
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Fast Support 24/7</a>
                          </h4>
                          <p class="mb-0">
                            We are quick to respond to your needs and available 24 hours a day.
                          </p>
                        </div>
                        <div
                          class="flex-shrink-0 position-relative order-1 order-lg-2"
                        >
                          <div class="icon-box">
                            <img src="img/icons/icon-09.png" alt="..." />
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      class="col-lg-4 text-center d-none d-lg-block my-md-5 my-xl-0 wow fadeInUp"
                      data-wow-delay="400ms"
                    >
                      <img src="img/content/why-choose-us.png" alt="..." />
                    </div>
                    <div
                      class="col-sm-6 col-lg-4 wow fadeInUp"
                      data-wow-delay="600ms"
                    >
                      <div class="d-flex me-lg-6 mb-1-9 mb-md-6">
                        <div class="flex-shrink-0 position-relative">
                          <div class="icon-box right">
                            <img src="img/icons/icon-10.png" alt="..." />
                          </div>
                        </div>
                        <div class="flex-grow-1 ms-1-6 ms-md-1-9">
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Best Pricing</a>
                          </h4>
                          <p class="mb-0">
                            Abovemarts has the best price value for products and services that is mouthwatering.
                          </p>
                        </div>
                      </div>
                      <div class="d-flex mb-1-9 mb-md-6 ms-lg-6">
                        <div class="flex-shrink-0 position-relative">
                          <div class="icon-box right">
                            <img src="img/icons/icon-11.png" alt="..." />
                          </div>
                        </div>
                        <div class="flex-grow-1 ms-1-6 ms-md-1-9">
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Security</a>
                          </h4>
                          <p class="mb-0">
                            Our system is secured and protected using DDos protection and SSL.
                          </p>
                        </div>
                      </div>
                      <div class="d-flex me-lg-6">
                        <div class="flex-shrink-0 position-relative">
                          <div class="icon-box right">
                            <img src="img/icons/icon-12.png" alt="..." />
                          </div>
                        </div>
                        <div class="flex-grow-1 ms-1-6 ms-md-1-9">
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Ultrafast Connect</a>
                          </h4>
                          <p class="mb-0">
                            AboveMarts assures customers of immediate connection after service.
                          </p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <span class="why-choose-shape1"></span>
            <img
              src="img/content/line-02.png"
              class="position-absolute left-5 bottom-10 ani-top-bottom d-none d-sm-block"
              alt="..."
            />
          </section>

          <section>
            <div class="container z-index-2 position-relative">
              <div class="section-heading">
                <h2>
                  5 Reasons To 
                  <span class="font-weight-400">Join Us Now </span>
                </h2>
              </div>
            </div>
            <div class="container-fluid">
              <div class="row justify-content-center z-index-2 position-relative">
                <div class="col-xxl-9">
                  <div class="row align-items-md-center">
                    <div
                      class="col-sm-6 col-lg-6 wow fadeInUp"
                      data-wow-delay="200ms"
                    >
                      <div class="d-flex ms-lg-6 mb-1-9 mb-md-6">
                        <div
                          class="flex-grow-1 text-lg-end ms-4 ms-lg-0 me-lg-1-9 order-2 order-lg-1"
                        >
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!"> Internet Driven Business</a>
                          </h4>
                          <p class="mb-0">
                            There are over 4.5 Billion Internet Users Worldwide
                          </p>
                          <p class="mb-0">
                            There are over 140 Million Internet Users in Nigeria.
                          </p>
                        </div>
                        <div
                          class="flex-shrink-0 position-relative order-1 order-lg-2"
                        >
                          {{-- <div class="icon-box">
                            <img src="img/icons/icon-07.png" alt="..." />
                          </div> --}}
                        </div>
                      </div>
                      <div class="d-flex mb-1-9 mb-md-6 me-lg-6">
                        <div
                          class="flex-grow-1 text-lg-end ms-4 ms-lg-0 me-lg-1-9 order-2 order-lg-1"
                        >
                        <h4 class="h5 mb-1 mb-md-3">
                          <a href="#!"> Global E-Commerce Opportunity</a>
                        </h4>
                        <p class="mb-0">
                          E-Commerce has made JEFF BEZOS the Richest Man in the World.
                        </p>
                        <p class="mb-0">
                          Amazon generates over $1.2 Billion daily i.e about $15,000 per Seconds.
                        </p>
                        </div>
                        <div
                          class="flex-shrink-0 position-relative order-1 order-lg-2"
                        >
                          {{-- <div class="icon-box">
                            <img src="img/icons/icon-08.png" alt="..." />
                          </div> --}}
                        </div>
                      </div>
                      <div class="d-flex mb-1-9 mb-md-0 ms-lg-6">
                        <div
                          class="flex-grow-1 text-lg-end ms-4 ms-lg-0 me-lg-1-9 order-2 order-lg-1"
                        >
                        <h4 class="h5 mb-1 mb-md-3">
                          <a href="#!"> Discounted Telecom Services</a>
                        </h4>
                        <p class="mb-0">
                          Nigerians spend over N3 Trillion+ on Airtime Recharge Annually.
                        </p>
                        <p class="mb-0">
                          MTN alone generated over N560 Billion+ from Airtime, Data, and SMS in Q1.
                        </p>
                        </div>
                        <div
                          class="flex-shrink-0 position-relative order-1 order-lg-2"
                        >
                          {{-- <div class="icon-box">
                            <img src="img/icons/icon-09.png" alt="..." />
                          </div> --}}
                        </div>
                      </div>
                    </div>
                   
                    <div
                      class="col-sm-6 col-lg-6 wow fadeInUp"
                      data-wow-delay="600ms"
                    >
                      <div class="d-flex me-lg-6 mb-1-9 mb-md-6">
                        <div class="flex-shrink-0 position-relative">
                          {{-- <div class="icon-box right">
                            <img src="img/icons/icon-10.png" alt="..." />
                          </div> --}}
                        </div>
                        <div class="flex-grow-1 ms-1-6 ms-md-1-9">
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Digital Asset Accumulation</a>
                          </h4>
                          <p class="mb-0">
                            If You Had $60/N9,000 Worth Of Bitcoin Digital Asset in March 2009.
                          </p>
                          <p class="mb-0">
                            You Would Have Worth $395 Million/N140 Billion+ by December 2017.                         
                          </p>
                        </div>
                      </div>
                      <div class="d-flex mb-1-9 mb-md-6 ms-lg-6">
                        <div class="flex-shrink-0 position-relative">
                          {{-- <div class="icon-box right">
                            <img src="img/icons/icon-11.png" alt="..." />
                          </div> --}}
                        </div>
                        <div class="flex-grow-1 ms-1-6 ms-md-1-9">
                          <h4 class="h5 mb-1 mb-md-3">
                            <a href="#!">Our Business Model</a>
                          </h4>
                          <p class="mb-0">
                            The Cooperative Business Model Is Over 500 Years Old.                          
                          </p>
                          <p class="mb-0">
                            The most remarkable form of Social Enterprise ever created.                          
                          </p>
                        </div>
                      </div>

                    </div>
                    <div class="col-lg-4"></div>

                    <div class="col-lg-4">
                      <a href="{{ route('register') }}" class="butn small" >Get Started</a>

                    </div>
                    <div class="col-lg-4"></div>
                  </div>
                </div>
              </div>
            </div>
            
          </section>
    